...
class ResCompanyLdap(models.Model):
    ...
    def _get_or_create_user(self, conf, login, ldap_entry):
        ...
        # raises AccessDenied when there is no user and create_user is off
        ...
        for mapping in this.group_mapping_ids:
            ...
            if operator(ldap_entry, mapping):
                ...
                groups.append(Command.link(mapping.group_id.id))
        if (
            ...
            and len(
                [
                    g[1]
                    for g in groups
                    if g[0] == Command.LINK and g[1] in essential_groups
                ]
            )
            != 1
        ):
