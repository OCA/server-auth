...
# Odoo 19: auth_ldap renamed the model class CompanyLDAP -> ResCompanyLdap
_company_ldap_class = _auth_ldap_ns + ".models.res_company_ldap.ResCompanyLdap"
...
class TestUsersLdapGroups(TransactionCase):
    ...
    def _fake_connection(self, extra=None):
        ...
        if extra:
            entry.update(extra)
        return FakeLdapConnection({"dc=users_ldap_groups,dc=example,dc=com": entry})
    ...
    def test_users_ldap_groups_ldap_returns_binary_data(self):
        ...
        with mock.patch(
            ...
            return_value=self._fake_connection(extra=binary),
        ):
            auth_info = self._authenticate()
        user = self.env["res.users"].sudo().browse(auth_info["uid"])
        groups = user.group_ids
