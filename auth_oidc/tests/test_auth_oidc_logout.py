...
def create_request(env, user_id, user_logout_func, redirect_request_func):
    with MockRequest(env) as request:
        ...
        request.redirect = redirect_request_func
        yield request
...
class TestOpenIDLogout(common.HttpCase):
    ...
    def test_skip_oidc_logout_for_user(self):
        ...
        with create_request(
            self.env, user.id, self.mock_logout_user, self.mock_redirect
        ):
            self.assertIsNotNone(OpenIDLogout().logout())
            self.assertEqual(LOGIN_PATH, self.redirect_url)
    ...
    def test_oidc_logout(self):
        ...
        with MockRequest(self.env) as request:
            ...
            self.assertIsNotNone(OpenIDLogout().logout())
            self.assertTrue(self.redirect_url.startswith(OIDC_BASE_LOGOUT_URL))
            self.assertFalse(self.is_local_direct)
            actual_components = urlparse(self.redirect_url)
    ...
    def test_oidc_logout_with_params(self):
        ...
        with create_request(
            ...
            self.assertIsNotNone(OpenIDLogout().logout(redirect_path))
            self.assertTrue(self.redirect_url.startswith(OIDC_BASE_LOGOUT_URL))
            actual_components = urlparse(self.redirect_url)
            self.assertEqual(OIDC_LOGOUT_PATH, actual_components.path)
            actual_params = dict(parse_qsl(actual_components.query))
    ...
    def test_oidc_logout_with_absolute_redirect_url(self):
        ...
        with create_request(
            ...
            self.assertIsNotNone(OpenIDLogout().logout(BASE_URL))
            self.assertTrue(self.redirect_url.startswith(OIDC_BASE_LOGOUT_URL))
            actual_components = urlparse(self.redirect_url)
            self.assertEqual(OIDC_LOGOUT_PATH, actual_components.path)
            actual_params = dict(parse_qsl(actual_components.query))
