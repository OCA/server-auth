...
class TestResUsers(TransactionCase):
    ...
    def test_check_password_crypt(self):
        """Il doit lever UserError si le mot de passe a déjà été utilisé."""
        rec_id = self._new_record()
        with self.assertRaises(UserError):
    ...
    def test_validate_pass_reset_error(self):
        """Doit lever UserError si reset dans le délai minimum."""
        rec_id = self._new_record()
        with self.assertRaises(UserError):
            rec_id._validate_pass_reset()

    def test_validate_pass_reset_allow(self):
        """Doit autoriser le reset hors délai minimum."""
        rec_id = self._new_record()
    ...
    def test_validate_pass_reset_zero(self):
        """Doit autoriser le reset quand minimum_hours <= 0."""
        rec_id = self._new_record()
