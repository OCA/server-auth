...
class ResUsers(models.Model):
    ...
    def _validate_pass_reset(self):
        """Validations avant de lancer l'envoi de l'email de réinitialisation.

        :raises: UserError si la réinitialisation n'est pas autorisée
    ...
    def _check_password_history(self, password):
        """Valide le mot de passe proposé par rapport à l'historique.

        :raises: UserError si le mot de passe a déjà été utilisé
        """
        crypt = self._crypt_context()
        pwd_params = self._get_all_password_params()
        for user in self:
            if not pwd_params["history"]:  # désactivé
                recent_passes = self.env["res.users.pass.history"]
            elif pwd_params["history"] < 0:  # illimité
    ...
    def _set_encrypted_password(self, uid, pw):
        """Sauvegarde le hash du mot de passe dans l'historique."""
    ...
    def action_reset_password(self):
        """Interdit la réinitialisation du mot de passe dans le délai minimum."""
