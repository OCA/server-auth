...
class TestAuthMethod(TransactionCase):
    ...
    def test_valid_token_with_audience_type_aud(self):
        with self._create_validator("val1", audience="client1,client2") as validator:
    ...
    def test_invalid_audience_with_audience_type_aud(self):
        with self._create_validator("val2", audience="client1,client2") as validator:
            validator.audience_type = "aud"
    ...
    def test_valid_token_with_custom_audience_type(self):
        with self._create_validator("val3", audience="read,write") as validator:
            ...
            token = jwt.encode(
                payload,
                "thesecret012345678901234567890123456789",
                algorithm="HS256",
            )
    ...
    def test_invalid_custom_audience_type(self):
        with self._create_validator("val4", audience="read") as validator:
            validator.audience_type = "custom"
            validator.audience_type_custom = "scope"
    ...
    def test_invalid_signature_rejected(self):
        with self._create_validator("val5", audience="client1") as validator:
            validator.audience_type = "aud"
            # PyJWT raises InsecureKeyLengthWarning on HMAC keys <32 bytes,
            # which the upstream test config treats as an error. Match the
            # 40-byte length of the helper's default key but with different
            # content so the signature actually mismatches the validator.
            token = self._create_token(
                audience="client1",
                key="wrongsecret098765432109876543210987654321",
            )
            with self.assertRaises(UnauthorizedInvalidToken):
                validator._decode(token)
