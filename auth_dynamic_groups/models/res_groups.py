...
class ResGroups(models.Model):
    ...
    def dynamic_method_formula_check(self, user):
        ...
        self.ensure_one()
        message_parms = self._get_message_parms(user)
        message_parms['condition'] = self.dynamic_group_condition
        satisfied = safe_eval(
            ...
            {'dynamic_group': self,
             ...
             'all': all,
             'filter': filter})
        message_parms['not_or_not_not'] = '' if satisfied else 'not '
        self._debug_message(
            _("User %(user)s does %(not_or_not_not)s satisfy formula %(condition)s"
    ...
    def dynamic_method_category_check(self, user):
        ...
        for category in self.partner_category_ids:
            if category in user.partner_id.category_id:
                message_parms['category'] = \
                    "%d:%s" % (category.id, category.display_name)
                self._debug_message(
                    _("User %(user)s has %(category)s defined in"
                      " dynamic group %(group)s"),
                    message_parms)
                return True
        self._debug_message(
            _("User %(user)s has no category defined in dynamic group %(group)s"),
            message_parms)
        return False
    ...
    def _get_message_parms(self, user):
        """Get standard message parms for debug messages."""
        return {
