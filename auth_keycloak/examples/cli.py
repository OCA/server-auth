...
import click
from common import DATA_FILE, USERINFO_PATH, USERS_PATH, VALIDATE_PATH, do_request
...
def user_info(ctx, **kw):
    ...
    url = urljoin(params["domain"], USERINFO_PATH.format(realm=params["realm"]))
    headers = {
...
def search_users(ctx, search=None, **kw):
    """Retrieve users info."""
    params = ctx.parent.params
    url = urljoin(params["domain"], USERS_PATH.format(realm=params["realm"]))
    if search:
...
def validate_token(ctx, **kw):
    ...
    url = urljoin(params["domain"], VALIDATE_PATH.format(realm=params["realm"]))
