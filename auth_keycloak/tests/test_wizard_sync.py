...
from .common import FAKE_TOKEN_RESPONSE, FAKE_USERS_RESPONSE, TestKeycloakWizBase
...
class TestWizard(TestKeycloakWizBase):
    ...
    def test_get_token(self):
        ...
        self.assertListEqual(sorted(parse.parse_qsl(request.body)), sorted(expected))
    ...
    def test_get_users(self):
        ...
        self.assertListEqual(responses.calls[1].response.json(), FAKE_USERS_RESPONSE)
    ...
    def test_sync_by_username(self):
        ...
        self.assertEqual(len(responses.calls), 2)
        self.assertEqual(
            action["domain"], [("id", "in", (self.user_donald + self.user_john).ids)]
        )
        self.assertEqual(
            self.user_donald.oauth_uid, "1feb89e6-76bd-44a1-ab5d-df28b6477e19"
        )
        self.assertEqual(self.user_donald.oauth_provider_id, self.provider)
        self.assertEqual(
            self.user_john.oauth_uid, "ef1d2e5d-1aad-4daf-858e-f246168a10ef"
        )
        self.assertEqual(self.user_john.oauth_provider_id, self.provider)
