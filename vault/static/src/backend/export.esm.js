...
const vaultExportService = {
    ...
    start(env, {vault_utils}) {
        ...
        class VaultExporter {
            ...
            async _export_json_entry(master_key, node) {
                ...
                for (const field of node.fields || [])
                    fields.push({
                        name: field.name,
                        value: await vault_utils.sym_decrypt(
                            master_key,
                            field.value,
                            field.iv
                        ),
            ...
            async _export_json(master_key, data) {
                ...
                const salt = vault_utils.generate_bytes(vault_utils.SaltLength).buffer;
                const iterations = vault_utils.Derive.iterations;
                const key = await vault_utils.derive_key(password, salt, iterations);
                ...
                return {
                    type: "encrypted",
                    iv: iv,
                    salt: vault_utils.toBase64(salt),
                    data: await vault_utils.sym_encrypt(key, content, iv),
        ...
        return new VaultExporter();
    },
};

registry.category("services").add("vault_export", vaultExportService);
