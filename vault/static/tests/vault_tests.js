...
odoo.define("vault.tests", function (require) {
  ...
  QUnit.module("vault", {
    ...
    QUnit.test("vault: Test asymmetric encryption", async function (assert) {
      ...
      assert.strictEqual(text, await utils.asym_decrypt(key.privateKey, crypted));
    });
    ...
    QUnit.test("vault: Importer/exporter", async function (assert) {
      ...
      const imported = await importer.import(master_key, "test.json", JSON.stringify(data));

      const exporter = new Exporter();
      const exported = await exporter.export(master_key, "test.json", JSON.stringify(imported));
      ...
      const tmp = JSON.parse(await utils.sym_decrypt(pass, exported.data, exported.iv));
