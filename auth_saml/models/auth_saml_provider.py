...
class AuthSamlProvider(models.Model):
    ...
    idp_metadata = fields.Text(
        ...
        help=(
            "Configuration for this Identity Provider."
            " Supplied by the provider, in XML format."
        ),
        required=True,
    )
    ...
    matching_attribute = fields.Char(
        ...
        required=True,
        help=(
            "Attribute to look for in the returned IDP"
            " response to match against an Odoo user."
        ),
    )
    ...
    autoredirect = fields.Boolean(
        ...
        default=False,
        help=(
            "Only the provider with the higher priority will be "
    ...
    authn_requests_signed = fields.Boolean(
        default=True,
        help=(
            "Indicates if the Authentication Requests sent by this SP "
