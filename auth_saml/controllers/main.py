...
class AuthSAMLController(http.Controller):
    ...
    def get_auth_request(self, pid):
        ...
        redirect_url = provider._get_auth_request(self._get_saml_extra_relaystate())
        if not redirect_url:
            raise Exception(
    ...
    def signin(self, req, **kw):
        ...
        with registry.cursor() as cr:
            try:
                ...
                credentials = env["res.users"].sudo().auth_saml(provider, saml_response)
                cr.commit()
                action = state.get("a")
                menu = state.get("m")
                url = "/"
    ...
    def saml_metadata(self, req, **kw):
        ...
        with registry.cursor() as cr:
            ...
            return request.make_response(
                client._metadata_string(valid, signed), [("Content-Type", "text/xml")]
            )
