...
class SAMLLogin(Home):
    def list_providers(self, with_autoredirect=False):
        try:
            ...
            providers = (
                request.env["auth.saml.provider"].sudo().search_read(domain)
            )
        except Exception as e:
            _logger.exception("SAML2: %s", e)
            providers = []
    ...
    def _saml_autoredirect(self):
        ...
        disable_autoredirect = (
            "disable_autoredirect" in request.params
            or "error" in request.params
        )
    ...
    def web_login(self, *args, **kw):
        ...
        if response.is_qweb:
            error = None
            error_code = request.params.get("saml_error")
            if error_code == "no-signup":
                error = _("Sign up is not allowed on this database.")
            elif error_code == "access-denied":
    ...
    def signin(self, req, **kw):
        ...
        with registry.cursor() as cr:
            try:
                ...
                credentials = (
                    env["res.users"].sudo().auth_saml(provider, saml_response)
                )
                cr.commit()
                action = state.get("a")
                menu = state.get("m")
                ...
                url = "/web/login?saml_error=no-signup"
