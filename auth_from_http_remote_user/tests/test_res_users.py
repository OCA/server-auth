...
class TestResUsers(TransactionCase):
    ...
    def test_login(self):
        ...
        res = res_users_obj.authenticate(common.get_db_name(), "admin", "admin", None)
        uid = res
        ...
        with self.assertRaises(AccessDenied):
            res_users_obj.authenticate(common.get_db_name(), "admin", token, None)
        ...
        with mock_cursor(self.cr):
            ...
            self.env["res.users"].sudo().check(common.get_db_name(), uid, token)
            # We are able to login with the new token
            res = res_users_obj.authenticate(common.get_db_name(), "admin", token, None)
            self.assertTrue(res)
