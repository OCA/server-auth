...
odoo.define("auth_u2f.u2f", function (require) {
    ...
    CrashManager.include({
        rpc_error: function (error) {
            if (
                error.data.name ===
                "odoo.addons.auth_u2f.models.http.U2FAuthenticationError"
            ) {
                return framework.redirect(
                    "/web/u2f/login?redirect=" +
                        encodeURIComponent(
    ...
    var FieldU2F = AbstractField.extend({
        ...
        _get_error_message: function () {
            ...
            if (code === 1) {
                ...
                return _t(
                    "A bad request was sent to your browser. This can happen when the URL\
 you're on is not web.base.url or when this page is not served over HTTPS."
        ...
        _render: function () {
            if (!this.u2f_error_code) {
                ...
                if (this.value) {
                    this.$el.html(
                        '<i class="fa fa-check-circle"></i> ' + _t("Scanned key.")
                    );
                } else {
                    this.$el.html(
                        '<i class="fa fa-spinner fa-spin"></i> ' +
                            _t(
                                "Insert your security key and tap the button or gold disk."
                            )
                    );
                }
            } else {
        ...
        _renderEdit: function () {
            ...
            this._rpc({
                ...
                u2f.register(
                    ...
                    0
                ); // Timeout of 0 seconds means don't timeout
            });
        },
    });
